(function () {
  ...
  const NAV = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: "emojiquotes", label: "情绪金句" },
        { id: "dailyplay", label: "日常吃喝玩乐" },
  ...
  function buildListSection(id, title, en, items) {
    const wrap = el("div", { class: "cols-2" });
    (items || []).forEach((t) => wrap.appendChild(el("div", { class: "news-line", text: t })));
    return section(id, title, en, wrap);
  }

  /* ---------- 情绪金句 ---------- */
  function buildEmojiquotes() {
    const card = el("div", { class: "card" });
    (D.emojiquotes || []).forEach((q, i) => {
      if (i > 0) card.appendChild(el("div", { style: "height:1px;background:var(--hair);margin:14px 0" }));
      card.appendChild(el("div", { class: "hero-quote", style: "font-size:17px;margin:0", text: q }));
    });
    return section("emojiquotes", "情绪金句 · 六则", "MOOD QUOTES", card);
  }

  /* ---------- 日常吃喝玩乐 ---------- */
  function buildDailyplay() {
    const col = el("div", { class: "col-1" });
    (D.dailyplay || []).forEach((p) => {
      col.appendChild(
        el("div", { class: "acard" }, [
          el("div", { class: "lot", text: p.scene }),
          el("div", { class: "note", style: "line-height:1.8", text: p.text })
        ])
      );
    });
    return section("dailyplay", "日常吃喝玩乐 · 八场", "DAILY LIFE", col);
  }
  ...
  function render() {
    ...
    content.appendChild(buildEmojiquotes());
    content.appendChild(buildDailyplay());
